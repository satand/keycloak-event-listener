...
class ProviderConfigTest {
    ...
    @SetEnvironmentVariable.SetEnvironmentVariables({
            @SetEnvironmentVariable(key = ProviderConfig.EXTERNAL_LDAP_FEDERATION_PROVIDER_URL, value = "ldap://localhost:3389"),
    ...
    void testEnvConstructor() {
        ...
        assertThat(configuration.getProviderUrl(), equalTo("ldap://localhost:3389"));
    ...
    void testExceptionOnMissingProviderUrlEnv() {
        IllegalArgumentException illegalArgumentException = Assertions.assertThrows(IllegalArgumentException.class, ProviderConfig::new);
        assertThat(illegalArgumentException.getMessage(), equalTo("The environment variable EXTERNAL_LDAP_FEDERATION_PROVIDER_URL is mandatory but is not present"));
    }
    ...
    void testAllArgsConstructor() {
        ProviderConfig configuration = new ProviderConfig("ldap://localhost:3389", "cn=admin,dc=ldap,dc=example,dc=com", "password", "ou=users,dc=ldap,dc=secondary,dc=example,dc=com", "employeeNumber=numero,title=titolo", "cn");
        assertThat(configuration.getProviderUrl(), equalTo("ldap://localhost:3389"));
        assertThat(configuration.getSecurityPrincipal(), equalTo("cn=admin,dc=ldap,dc=example,dc=com"));
        assertThat(configuration.getSecurityCredentials(), equalTo("password"));
        assertThat(configuration.getExternalAttributes().size(), is(2));
    ...
    @Test
    void testSplitStringIntoMap() {
        String configurationValue = "employeeNumber=numero,title=titolo";
        Map<String, String> actualValue = Arrays.stream(configurationValue.split(",")).map(s -> s.split("=")).collect(Collectors.toMap(k -> k[0], v -> v[1]));


        assertThat(actualValue.get("employeeNumber"), equalTo("numero"));
        assertThat(actualValue.get("title"), equalTo("titolo"));
    }
